...
static void on_receive_byte(uint8_t b) {
    ...
    if (rx_count < I2C_CTRL_HEADER_LEN) {
        ...
        if (rx_count == I2C_CTRL_HEADER_LEN) {
            ...
            if (type == I2C_CTRL_TYPE_SET && breq == REQ_SET_ALL_PARAMS &&
                wlen >= WIRE_BULK_PARAMS_MIN_SIZE && wlen <= sizeof(WireBulkParams)) {
...
static void on_finish(void) {
    ...
    if (rx_count > 0) {
        ...
        if (rx_count == expect &&
            ...
            resp_hdr[0] = rx_discard ? rx_discard_status : CTRL_STATUS_FRAME_ERROR;
            resp_hdr[1] = 0;
            resp_hdr[2] = 0;
            resp_len    = 0;
            resp_bulk   = false;
...
static uint8_t map_dispatch_status(CtrlDispatchResult r) {
    switch (r) {
...
void i2c_ctrl_poll(void) {
    ...
    // Keep a slow controller's bulk read from being reaped as stale.
    if (resp_ready && resp_bulk) vendor_bulk_touch(CTRL_SOURCE_I2C);
    ...
    if (!cur_valid) {
        ...
        memcpy(cur_payload, (const void *)rx_payload, sizeof(cur_payload));
        cur_bulk = rx_bulk;
    ...
    uint16_t wlen = (uint16_t)(cur_hdr[6] | (cur_hdr[7] << 8));
    ...
    if (r == CTRL_DISPATCH_OK && type == I2C_CTRL_TYPE_GET) {
        ...
        park_response(map_dispatch_status(r), NULL, 0, false);
    }
}
...
uint8_t i2c_ctrl_validate(const I2cCtrlConfig *cfg) {
    ...
    if (i2c_instance_index(cfg->sda_pin) != i2c_instance_index(cfg->scl_pin))
        return PIN_CONFIG_INVALID_PIN;
    if (is_pin_in_use(cfg->sda_pin, 0xFF) || is_pin_in_use(cfg->scl_pin, 0xFF))
...
void i2c_ctrl_init(const I2cCtrlConfig *cfg) {
    live_cfg = *cfg;
    live = false;
    if (cfg->enabled && i2c_ctrl_validate(cfg) == PIN_CONFIG_SUCCESS) {
        i2c_ctrl_up(cfg);
    }
    // On validation failure the interface stays down but keeps its stored
    // config; REQ_GET_CTRL_IFACE_STATUS exposes live=false to the host.
}
...
void i2c_ctrl_get_live_config(I2cCtrlConfig *out) { *out = live_cfg; }
